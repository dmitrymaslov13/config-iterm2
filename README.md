...
## Про комментарии в plist

XML-комментарии в `com.googlecode.iterm2.plist` синтаксически допустимы и не
должны ломать чтение файла. Но iTerm2 при сохранении настроек может
перезаписать plist и удалить комментарии, а иногда сохранить файл в бинарном
формате. Поэтому пояснения к осознанным настройкам хранятся здесь, в README.

## Осознанные настройки

- `Normal Font: Monaco 14` - увеличивает основной размер шрифта относительно
  дефолтного `Monaco 12`.
- `Terminal Type: xterm-256color` - включает 256-цветный terminal type для
  корректной работы цветов в shell, tmux, Neovim и CLI-приложениях.
- `Custom Directory: Recycle` - новые сессии переиспользуют текущую рабочую
  директорию вместо старта только из домашней директории.
- `Working Directory: /Users/dmitrymaslov13` - базовая директория профиля на
  этом Mac.
- `Keyboard Map -> Cmd+Shift+Space -> [32;10u` - отправляет CSI-u escape
  sequence, чтобы Neovim мог распознать сочетание как `<D-S-Space>`.
- `Jobs to Ignore: rlogin, ssh, slogin, telnet` - эти процессы не считаются
  причиной дополнительно предупреждать при закрытии сессии.
